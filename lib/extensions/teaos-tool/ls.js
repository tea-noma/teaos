...
teaos.declare({
  ...
  func: (argv) => {
    ...
    if(arg3 === 'resources'){
      const recursive = argv.indexOf('-r') !== -1;
      const resourceArgs = argv.slice(4).filter((value) => value !== '-r');
      const resourcePath = resourceArgs.length ? resourceArgs[0] : null;
      const teaosResourcePrefixs = (process.env.TEAOS_RESOURCE_PREFIXS || '')
        .split(',')
        .map((value) => value.trim())
        .filter((value) => value.length);
      console.log(teaos.resources({
        cwd: process.cwd(),
        path: resourcePath,
        recursive,
        teaosResourcePrefixs
      }));
      return;
    }
